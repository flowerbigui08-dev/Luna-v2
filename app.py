...
def obtener_fechas_especiales(anio_objetivo):
    ...
    # Cuenta del Omer (Inicia 16 de Aviv)
    omer_inicio = dia_1 + timedelta(days=15)
    ...
    for semana in calendar.Calendar(6).monthdayscalendar(anio, mes_id):
        ...
        for dia in semana:
            if dia == 0: fila += "<td></td>"
                ...
                elif f_actual == esp["omer_ini"].date():
                    b_style, icons = "border: 2px solid #9370DB; background: #1e122b; border-radius: 10px;", "🌾"
                elif f_actual == esp["omer_fin"].date():
                    b_style, icons = "border: 2px solid #9370DB; background: #1e122b; border-radius: 10px;", "🕊️"
                elif esp["omer_ini"].date() < f_actual < esp["omer_fin"].date():
                    b_style = "border: 2px solid #9370DB; background: #1e122b; border-radius: 10px;"
                ...
                if dia in fases_dict and "🌘" not in icons and "🍷" not in icons and "🌾" not in icons: 
                    icons += iconos_fases[fases_dict[dia]]
...
# --- PESTAÑA 2: AÑO ---
with tab_anio:
    anio_full = st.number_input("Año", 2024, 2030, hoy_sv.year, key="anio_f_full")
        ...
        for sem in calendar.Calendar(6).monthdayscalendar(anio_full, m):
            ...
            for d in sem:
                if d == 0: mes_html += "<td></td>"
                    ...
                    elif esp_a["omer_ini"].date() <= f_l <= esp_a["omer_fin"].date(): est += "border: 1.5px solid #9370DB; background: rgba(147,112,219,0.2); border-radius: 4px;"
...
with tab_simb:
    ...
    with c1:
        simbolos = [
            ("🌾", "<b>Día 1 del Omer:</b> Inicio de la cuenta (16 de Aviv)."),
            ("🟣", "<b>Cuenta del Omer:</b> Conteo de 50 días hacia Shavuot."),
            ("🕊️", "<b>Día 50 (Pentecostés):</b> Final de la cuenta del Omer."),
