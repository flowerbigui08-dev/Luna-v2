...
# COLOR GRIS ELEGANTE SELECCIONADO
color_gris_elegante = "#2c303c"
...
with tab_mes:
    c1, c2 = st.columns(2)
    with c1: anio = st.number_input("Año", 2024, 2030, hoy_sv.year, key="y_elegance")
    with c2: mes_id = st.number_input("Mes", 1, 12, hoy_sv.month, key="m_elegance")
    ...
    for semana in calendar.Calendar(6).monthdayscalendar(anio, mes_id):
        ...
        for dia in semana:
            if dia == 0: fila += "<td></td>"
                ...
                ico, b_style = "", f"border: 1px solid #4a4e5a; background: {color_gris_elegante};"
    ...
    <div class="info-box">
        ...
        <p style="margin:0; font-size:15px;">✅ Hoy | 🌑 Conjunción | 🌘 Celebración | 🌕 Luna Llena</p>
    </div>
    <div class="info-box">
        <p style="color:#FF8C00; font-weight:bold; margin-bottom:8px;">Próxima Conjunción:</p>
...
with tab_anio:
    anio_f = st.number_input("Año", 2024, 2030, hoy_sv.year, key="a_elegance", label_visibility="collapsed")
    ...
    for m in range(1, 13):
        ...
        m_h = f"<div style='background:{color_gris_elegante}; padding:8px; border-radius:8px; border:1px solid #4a4e5a;'>"
        ...
        for sem in calendar.Calendar(6).monthdayscalendar(anio_f, m):
            ...
            for d in sem:
                if d == 0: m_h += "<td></td>"
                    ...
                    if d in cs: style += "border:1px solid #FF8C00; background:rgba(255,140,0,0.25); border-radius:3px;"
