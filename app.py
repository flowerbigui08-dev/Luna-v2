...
st.markdown("""
    ...
    .time-data { color: #ddd; font-size: 15px; font-weight: 500; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin-top: 2px; }
...
with tab_mes:
    ...
    if conjs:
        ...
        <div class="conjunction-card">
            <p style="color:#FF8C00; font-weight:bold; font-size:16px; margin-bottom:5px;">Próxima Conjunción ({meses_completos[mes_m-1]}):</p>
...
with tab_anio:
    ...
    # CSS para Cuadrícula de 3 columnas
    grid_html = """
    <div style='display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; padding: 10px;'>
    <style>
        @media (max-width: 800px) { div { grid-template-columns: repeat(2, 1fr) !important; } }
        @media (max-width: 500px) { div { grid-template-columns: 1fr !important; } }
        .mes-box { background:#1a1c23; padding:8px; border-radius:10px; border:1px solid #333; color:white; }
        .mes-title { color:#FF8C00; font-weight:bold; text-align:center; font-size:13px; margin-bottom:5px; }
        .mes-table { width:100%; font-size:10px; text-align:center; border-collapse:collapse; }
        .mes-table td { padding: 2px; }
    </style>
    """
    for m in range(1, 13):
        ...
        for sem in calendar.Calendar(6).monthdayscalendar(anio_f, m):
            ...
            for d in sem:
                if d == 0: mes_html += "<td></td>"
                    ...
                    if f_l == esp_a["n13"]: est = "border: 1px solid #FF0000; border-radius: 3px; background: #2c0a0a;"
