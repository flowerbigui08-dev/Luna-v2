...
# ESTILOS CSS ACTUALIZADOS (Colores más sutiles)
st.markdown("""
    <style>
    ...
    for semana in cal.monthdayscalendar(anio, mes_id):
        ...
        for dia in semana:
            if dia == 0: 
                ...
                icons = ""
                # Color de número normal: Gris claro/blanco para que se vea en fondo oscuro
                n_color = "#e0e0e0" 
                b_style = "border: 1px solid #333; border-radius: 12px; background: #16181d;"
                ...
                # CAMBIO: Hoy ahora es un rojo sutil (Coral) para combinar con los encabezados
                if dia == hoy_sv.day and mes_id == hoy_sv.month and anio == hoy_sv.year:
                    b_style = "border: 2px solid #FF4B4B; background: rgba(255,75,75,0.1); border-radius: 12px;"
                    n_color = "#FF4B4B"
                ...
                fila += f"""<td style='border:none; padding:4px;'><div style='{b_style} height: 75px; padding: 6px; box-sizing: border-box;'>
                        <div style='font-weight:bold; font-size:14px; color:{n_color};'>{dia}</div>
...
with tab_anio:
    ...
    st.markdown("<div style='text-align:center; background:rgba(255,140,0,0.1); border:1px solid #FF8C00; padding:8px; border-radius:10px; color:white; margin:10px auto; max-width:250px; font-size:14px;'>🟧 Celebración Luna Nueva</div>", unsafe_allow_html=True)
        ...
        for semana in calendar.Calendar(6).monthdayscalendar(anio_full, m):
            ...
            for dia in semana:
                if dia == 0:
                    ...
                    txt_c = "#e0e0e0" # Color sutil para números en anual
                    ...
                    if dia == hoy_sv.day and m == hoy_sv.month and anio_full == hoy_sv.year:
                        inner_style += "border: 1.5px solid #FF4B4B; background: rgba(255,75,75,0.15); border-radius: 4px;"
                        txt_c = "#FF4B4B"
                    mes_html += f"<td><div style='{inner_style} color:{txt_c};'>{dia}</div></td>"
            mes_html += "</tr>"
        mes_html += "</table></div>"
        grid_html += mes_html
    
    grid_html += "</div>"
    components.html(f"<style>body{{background:#0e1117; margin:0; padding-bottom:50px;}}</style>{grid_html}", height=1450)

# PIE DE PÁGINA (Simbología y Créditos)
st.markdown(f"""
    <div class="info-box">
        <p style="color:#FF8C00; font-weight:bold; margin-bottom:15px; font-size:17px;">Simbología:</p>
        <div class="info-line"><span style="color:#FF4B4B; font-size:20px; margin-right:15px; width:30px; text-align:center;">■</span> Hoy (Día actual)</div>
    ...
    <div style="margin-top: 30px; padding: 15px; border-top: 1px solid #333; text-align: center;">
        <p style="color: #666; font-size: 12px;"><b>Respaldo Científico:</b> Skyfield & NASA Ephemeris.</p>
        <p style="color: #888; font-size: 16px; font-style: italic; margin-top: 10px; font-weight: bold;">Voz de la Tórtola, Nejapa.</p>
    </div>
    """, unsafe_allow_html=True)
