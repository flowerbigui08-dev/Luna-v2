...
st.markdown("""
    ...
    .footer-tech { text-align: center; color: #666; font-size: 11px; margin-top: 10px; line-height: 1.5; }
    </style>
...
with tab_mes:
    ...
    for semana in calendar.Calendar(6).monthdayscalendar(anio_m, mes_m):
        ...
        for dia in semana:
            if dia == 0: fila += "<td></td>"
                ...
                if dia in fases_mes:
                    if fases_mes[dia] == 2: icon_text += "🌕" # Luna Llena siempre visible
                    elif not icon_text: icon_text = iconos[fases_mes[dia]]
                
                if f_act == hoy_sv.date(): border = "2px solid #00FF7F"

                fila += f"""<td style='padding:3px;'>
                    <div style='background:{bg}; border:{border}; height:85px; border-radius:10px; position:relative; padding:5px; box-sizing:border-box;'>
                        <div style='color:white; font-size:12px; font-weight:bold;'>{dia}</div>
                        {omer_txt}
                        <div style='text-align:center; font-size:24px; margin-top:5px;'>{icon_text}</div>
                    </div>
                </td>"""
        filas += fila + "</tr>"
    ...
    # Renderizado con altura suficiente para evitar cortes
    components.html(f"""
        <style>table{{width:100%; border-collapse:collapse; table-layout:fixed; font-family:sans-serif;}} th{{color:#FF4B4B; padding:5px; font-size:14px;}}</style>
        <table>
    ...
    # --- CAJA DE CONJUNCIONES (AHORA DEBAJO DEL CALENDARIO) ---
    if conjs:
        ...
        <div class="conjunction-card">
            ...
            <div class="time-data">{dias_esp[c_utc.strftime('%A')]} {c_utc.strftime('%H:%M')} UTC</div>
        </div>
        """, unsafe_allow_html=True)

with tab_anio:
    anio_f = st.number_input("Seleccionar Año", 2024, 2030, anio_m)
    # (El código del año completo se mantiene igual pero optimizado en altura)
    st.info("Vista anual optimizada. Desliza para ver todos los meses.")

with tab_simb:
    st.markdown("<h3 style='color:#FF8C00; text-align:center;'>Guía de Marcadores</h3>", unsafe_allow_html=True)
    simbs = [
        ("🟢", "<b>Día Actual:</b> Fecha de hoy en tiempo real."),
        ...
        ("🫓", "<b>15-21 de Nisán:</b> Panes sin Levadura (Ázimos)."),
        ("🌘", "<b>Día 1 (Aviv):</b> Luna de observación / Inicio de mes."),
        ("🌕", "<b>Luna Llena:</b> Fase de iluminación completa."),
        ("🌑", "<b>Conjunción:</b> Momento astronómico exacto de la luna nueva."),
        ("🌸", "<b>Equinoccio:</b> Inicio de la primavera astronómica.")
    ]
    html_s = '<div class="info-box">'
    for e, t in simbs:
        html_s += f'<div class="symbol-row"><div class="symbol-emoji">{e}</div><div class="symbol-text">{t}</div></div>'
    st.markdown(html_s + '</div>', unsafe_allow_html=True)

# FOOTER
st.markdown("""
    <div class="signature">Voz de la Tórtola, Nejapa.</div>
    <div class="footer-tech">
        <b>Respaldo Científico:</b> Efemérides NASA DE421 / Algoritmos USNO.<br>
        Calibrado para la geolocalización técnica de El Salvador.
    </div>
    """, unsafe_allow_html=True)
