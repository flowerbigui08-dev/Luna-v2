...
st.markdown("""
    ...
    .sunset-table { width: 100%; border-collapse: collapse; margin-top: 10px; color: white; font-size: 14px; }
    .sunset-table td, .sunset-table th { border: 1px solid #333; padding: 8px; text-align: center; }
    .sunset-table th { background: #2c1a0a; color: #FF8C00; }
...
with tab_mes:
    ...
    components.html(f"<style>table{{width:100%; border-collapse:collapse; table-layout:fixed;}} th{{color:#FF4B4B; padding-bottom:5px; text-align:center; font-weight:bold; font-size:14px;}}</style><table><tr><th>D</th><th>L</th><th>M</th><th>M</th><th>J</th><th>V</th><th>S</th></tr>{filas_html}</table>", height=440)
...
# --- PESTAÑA ANUAL (CON OCASOS Y NOTAS) ---
with tab_anio:
    ...
    for m in range(1, 13):
        ...
        mes_html = f"<div style='background:#1a1c23; padding:10px; border-radius:10px; border:1px solid #333; color:white;'><div style='color:#FF8C00; font-weight:bold; text-align:center; margin-bottom:5px;'>{meses_completos[m-1]}</div><table style='width:100%; font-size:11px; text-align:center;'>"
        for sem in calendar.Calendar(6).monthdayscalendar(anio_full, m):
            ...
            for d in sem:
                if d == 0: mes_html += "<td></td>"
                    ...
                    if d == hoy_sv.day and m == hoy_sv.month: estilo += "border: 1px solid #00FF7F; background: rgba(0,255,127,0.2); border-radius:4px;"
    ...
    # 2. CUADRO DE OCASOS ANUAL
    st.markdown(f"### 🌅 Horarios de Ocaso {anio_full} (El Salvador)")
    tabla_sol = "<table class='sunset-table'><tr><th>Mes</th><th>Día 1</th><th>Día 15</th></tr>"
    for m in range(1, 13):
        tiempos = []
        for d in [1, 15]:
            t_s = ts.from_datetime(tz_sv.localize(datetime(anio_full, m, d, 18, 0)))
            t_e, y_e = almanac.find_discrete(t_s - timedelta(hours=6), t_s + timedelta(hours=6), almanac.sunrise_sunset(eph, loc_sv))
            hora = "---"
            for ti, yi in zip(t_e, y_e):
                if yi == 0: hora = ti.astimezone(tz_sv).strftime('%I:%M %p')
            tiempos.append(hora)
        tabla_sol += f"<tr><td>{meses_completos[m-1]}</td><td>{tiempos[0]}</td><td>{tiempos[1]}</td></tr>"
    st.markdown(tabla_sol + "</table>", unsafe_allow_html=True)

    # 3. ÁREA DE NOTAS
    st.markdown("---")
    st.text_area("✍️ Notas y Observaciones Teonómicas:", placeholder="Escribe aquí tus apuntes sobre las festividades o avistamientos...", height=150)

# PIE DE PÁGINA
st.markdown("""
    <div class="nasa-footer">
        <p style="color: #666; font-size: 12px;"><b>Respaldo Científico:</b> Cálculos Skyfield/NASA.</p>
